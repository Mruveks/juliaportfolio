...
function PortfolioCard({ item }: { item: PortfolioItem }) {
  return (
    <article className="group relative bg-[#FDFAF6] rounded-2xl overflow-hidden border border-[#DDD6CB] transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-[#1A1714]/8">
      <div className="relative aspect-[4/3] bg-[#E4DDD2] overflow-hidden">
        {item.videoUrl ? (
          <iframe src={item.videoUrl} title={item.title} className="w-full h-full" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
        ) : item.image ? (
          <Image src={item.image} alt={item.title} fill className="object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
        ) : (
        ...
        <div className="absolute top-3 left-3">
          <span className="bg-[#1A1714]/75 text-white text-xs font-medium px-3 py-1.5 rounded-full backdrop-blur-sm">{item.category}</span>
        </div>
      </div>
      <div className="p-6">
        <h3 className="font-semibold text-[#1A1714] text-lg mb-2 leading-snug">{item.title}</h3>
        <p className="text-[#1A1714]/55 text-sm leading-relaxed">{item.description}</p>
      </div>
    </article>
  );
}
