    ...
    <section id="pricing" className="py-24 md:py-32 bg-[#F3EDE3]">
      <div className="max-w-[1200px] mx-auto px-6 md:px-10">
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-sm font-medium text-[#B5926A] uppercase tracking-widest">Pakiety</span>
        </div>
            ...
            >
              ...
              <div className="mb-6">
                ...
                <div className="flex items-baseline gap-1 mb-3">
                  <span className={`text-4xl font-bold ${plan.highlighted ? "text-white" : "text-[#1A1714]"}`}
                    style={{ fontFamily: '"Playfair Display", Georgia, serif' }}>
                    {plan.price}
                  </span>
                  {plan.price !== "Wycena" && <span className={`text-sm ${plan.highlighted ? "text-white/50" : "text-[#8A8078]"}`}>zł</span>}
                  <span className={`text-sm ml-1 ${plan.highlighted ? "text-white/50" : "text-[#8A8078]"}`}>{plan.period}</span>
                </div>
              ...
              <ul className="flex-1 space-y-3 mb-8">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-3 text-sm">
                    <svg className={`w-4 h-4 mt-0.5 shrink-0 ${plan.highlighted ? "text-[#B5926A]" : "text-[#B5926A]"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
